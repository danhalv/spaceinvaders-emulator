...
void machine_update_state(machine_t *machine) {
	...
	while(cycle_count <= MACHINE_CYCLES_PER_FRAME) {
		...
		//i8080_step function skipped these instructions as they reference external hardware
		if(*opcode == 0xdb) {			//IN
			...
			switch(port) {
				...
				case 1:	machine->cpu.a = machine->in_port1;
						break;

				case 2:	machine->cpu.a = machine->in_port2;
						break;
			...
			switch(port) {
				...
				case 2: machine->shift_offset = machine->cpu.a & 0x7;	
						break;
				
				case 4:	machine->shift0 = machine->shift1;
