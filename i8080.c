...
//returns memory reference M, which is the memory byte at address made up of H and L registers
...
static void set_flags_zsp(i8080_t *state, uint8_t byte) {
	...
	state->cb.flags.p = should_set_parity_bit(byte, sizeof(byte));
}
...
static int8_t sub_twos_comp_and_set_flags(i8080_t *state, const int8_t minuend, const int8_t subtrahend) {
	...
	state->cb.flags.c = ~state->cb.flags.c;		//opposite of add
...
void i8080_daa(i8080_t *state) {
	...
	//** step 1 **
	if(least_significant_4 > 9 || state->cb.flags.ac == 1) 
		value_to_add = 0x06;
	...
	state->cb.flags.ac = ((lbits_a + lbits_vta) & 0xf0) != 0;
	...
	state->cb.flags.z = (0 == state->a);	
	state->cb.flags.s = (0x80 & state->a) != 0;
	state->cb.flags.p = should_set_parity_bit(state->a, sizeof(state->a));
...
void i8080_adc(i8080_t *state, const uint8_t *reg) {
	...
	const uint8_t addend = *reg + (state->cb.flags.c != 0);
...
void i8080_sbb(i8080_t *state, const uint8_t *reg) {
	...
	const uint8_t subtrahend = *reg + (state->cb.flags.c != 0);
...
void i8080_ana(i8080_t *state, const uint8_t *reg) {
	...
	state->cb.flags.ac = (0x08 & (state->a | result)) != 0;
	set_flags_zsp(state, result);
...
void i8080_push(i8080_t *state, const regpair_t pair) {
	...
	/*
	i8080_write_byte(state, state->sp-1, *pair.first);
	i8080_write_byte(state, state->sp-2, *pair.second);
...
void i8080_dad(i8080_t *state, const regpair_t pair, uint16_t *sp) {
	...
	*/
	...
	uint32_t result = hl + addend;

	state->cb.flags.c = ((result & 0xffff0000) != 0);	
...
void i8080_aci(i8080_t *state, uint8_t byte) {
	...
	const uint8_t addend = byte + (state->cb.flags.c != 0);
...
void i8080_sbi(i8080_t *state, uint8_t byte) {
	...
	uint8_t subtrahend = byte + (state->cb.flags.c != 0);
...
void i8080_cpi(i8080_t *state, uint8_t byte) {
	...
	const int16_t result = state->a - byte;
	
	state->cb.flags.c = (0x100 & result) != 0;
    state->cb.flags.ac = (~(state->a ^ result ^ byte) & 0x10) != 0;
	//state->cb.flags.ac = ((0xf & (int8_t)state->a) - (0xf & (int8_t)byte)) < 0;
	set_flags_zsp(state, result);	
...
void i8080_call(i8080_t *state, uint8_t low, uint8_t high) {
	...
	const uint16_t next_pc = state->pc + 3;	//pc of next instruction
...
void i8080_step(i8080_t *state) {
	...
	regpair_t PSW = {&state->a, &state->cb.byte};
	...
	switch(*opcode) {
		...
		case 0x30:	i8080_nop(state);											break;	//NOP
		case 0x31:	i8080_lxi(state, (regpair_t){}, SP, opcode[1], opcode[2]);	break;	//LXI SP, d16
		case 0x32:	i8080_sta(state, opcode[1], opcode[2]); 					break;	//STA d16
		case 0x33:	i8080_inx(state, (regpair_t){}, SP);						break;	//INX SP, d16
		case 0x34:	i8080_inr(state, M);									break;	//INR M
		case 0x35:	i8080_dcr(state, M);										break;	//DCR M
		case 0x36:	i8080_mvi(state, M, opcode[1]);								break;	//MVI M, d8
		case 0x37:	i8080_stc(state);											break;	//STC
		case 0x38:	i8080_nop(state);											break;	//NOP
		case 0x39:	i8080_dad(state, (regpair_t){}, SP);						break;	//DAD SP, d16
		case 0x3a:	i8080_lda(state, opcode[1], opcode[2]);						break;	//LDA d16
		case 0x3b:	i8080_dcx(state, (regpair_t){}, SP);						break;	//DCX SP
		case 0x3c:	i8080_inr(state, A);									break;	//INR A
		case 0x3d:	i8080_dcr(state, A);										break;	//DCR A
		case 0x3e:	i8080_mvi(state, A, opcode[1]);								break;	//MVI A, d8
		case 0x3f:	i8080_cmc(state);											break;	//CMC

		case 0x40:	i8080_mov(state, B, B);										break;	//MOV B, B
		case 0x41:	i8080_mov(state, B, C);										break;	//MOV B, C
		case 0x42:	i8080_mov(state, B, D);										break;	//MOV B, D
		case 0x43:	i8080_mov(state, B, E);										break;	//MOV B, E
		case 0x44:	i8080_mov(state, B, H);										break;	//MOV B, H
		case 0x45:	i8080_mov(state, B, L);										break;	//MOV B, L
		case 0x46:	i8080_mov(state, B, M);										break;	//MOV B, M
		case 0x47:	i8080_mov(state, B, A);										break;	//MOV B, A
		case 0x48:	i8080_mov(state, C, B);										break;	//MOV C, B
		case 0x49:	i8080_mov(state, C, C);										break;	//MOV C, C
		case 0x4a:	i8080_mov(state, C, D);										break;	//MOV C, D
		case 0x4b:	i8080_mov(state, C, E);										break;	//MOV C, E
		case 0x4c:	i8080_mov(state, C, H);										break;	//MOV C, H
		case 0x4d:	i8080_mov(state, C, L);										break;	//MOV C, L
		case 0x4e:	i8080_mov(state, C, M);										break;	//MOV C, M
		case 0x4f:	i8080_mov(state, C, A);										break;	//MOV C, A

		case 0x50:	i8080_mov(state, D, B);										break;	//MOV D, B 
		case 0x51:	i8080_mov(state, D, C);										break;	//MOV D, C
		case 0x52:	i8080_mov(state, D, D);										break;	//MOV D, D
		case 0x53:	i8080_mov(state, D, E);										break;	//MOV D, E
		case 0x54:	i8080_mov(state, D, H);										break;	//MOV D, H
		case 0x55:	i8080_mov(state, D, L);										break;	//MOV D, L
		case 0x56:	i8080_mov(state, D, M);										break;	//MOV D, M
		case 0x57:	i8080_mov(state, D, A);										break;	//MOV D, A
		case 0x58:	i8080_mov(state, E, B);										break;	//MOV E, B
		case 0x59:	i8080_mov(state, E, C);										break;	//MOV E, C
		case 0x5a:	i8080_mov(state, E, D);										break;	//MOV E, D
		case 0x5b:	i8080_mov(state, E, E);										break;	//MOV E, E
		case 0x5c:	i8080_mov(state, E, H);										break;	//MOV E, H
		case 0x5d:	i8080_mov(state, E, L);										break;	//MOV E, L
		case 0x5e:	i8080_mov(state, E, M);										break;	//MOV E, M
		case 0x5f:	i8080_mov(state, E, A);										break;	//MOV E, A

		case 0x60:	i8080_mov(state, H, B);										break;	//MOV H, B
		case 0x61:	i8080_mov(state, H, C);										break;	//MOV H, C
		case 0x62:	i8080_mov(state, H, D);										break;	//MOV H, D
		case 0x63:	i8080_mov(state, H, E);										break;	//MOV H, E
		case 0x64:	i8080_mov(state, H, H);										break;	//MOV H, H
		case 0x65:	i8080_mov(state, H, L);										break;	//MOV H, L
		case 0x66:	i8080_mov(state, H, M);										break;	//MOV H, M
		case 0x67:	i8080_mov(state, H, A);										break;	//MOV H, A
		case 0x68:	i8080_mov(state, L, B);										break;	//MOV L, B
		case 0x69:	i8080_mov(state, L, C);										break;	//MOV L, C
		case 0x6a:	i8080_mov(state, L, D);										break;	//MOV L, D
		case 0x6b:	i8080_mov(state, L, E);										break;	//MOV L, E
		case 0x6c:	i8080_mov(state, L, H);										break;	//MOV L, H
		case 0x6d:	i8080_mov(state, L, L);										break;	//MOV L, L
		case 0x6e:	i8080_mov(state, L, M);										break;	//MOV L, M
		case 0x6f:	i8080_mov(state, L, A);										break;	//MOV L, A

		case 0x70:	i8080_mov(state, M, B);										break;	//MOV M, B
		case 0x71:	i8080_mov(state, M, C);										break;	//MOV M, C
		case 0x72:	i8080_mov(state, M, D);										break;	//MOV M, D
		case 0x73:	i8080_mov(state, M, E);										break;	//MOV M, E
		case 0x74:	i8080_mov(state, M, H);										break;	//MOV M, H
		case 0x75:	i8080_mov(state, M, L);										break;	//MOV M, L
		case 0x76:						  										break;	//HLT ---unimplemented
		case 0x77:	i8080_mov(state, M, A);										break;	//MOV M, A
		case 0x78:	i8080_mov(state, A, B);										break;	//MOV A, B
		case 0x79:	i8080_mov(state, A, C);										break;	//MOV A, C
		case 0x7a:	i8080_mov(state, A, D);										break;	//MOV A, D
		case 0x7b:	i8080_mov(state, A, E);										break;	//MOV A, E
		case 0x7c:	i8080_mov(state, A, H);										break;	//MOV A, H
		case 0x7d:	i8080_mov(state, A, L);										break;	//MOV A, L
		case 0x7e:	i8080_mov(state, A, M);										break;	//MOV A, M
		case 0x7f:	i8080_mov(state, A, A);										break;	//MOV A, A
		...
		case 0xf1:	i8080_pop(state, PSW);										break;
		...
		case 0xf5:	i8080_push(state, PSW);										break;
...
void i8080_print(i8080_t *state) {
	...
	printf(
		...
		state->cb.flags.z, state->cb.flags.s, state->cb.flags.p, state->cb.flags.c, state->cb.flags.ac,	//flags
		state->cycles
	);
	printf("\n");
}
